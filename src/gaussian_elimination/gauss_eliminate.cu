...
int main(int argc, char** argv)
{
    ...
    printf("Run Time: CPU run time = %0.5f s. \n", (float)(stop.tv_sec - start.tv_sec + (stop.tv_usec - start.tv_usec)/(float)1000000));
...
void gauss_eliminate_on_device(const Matrix A, Matrix U)
{
    ...
    cudaMalloc((void **)&U_d, MATRIX_SIZE * MATRIX_SIZE * sizeof(float));
    cudaMemcpy(U_d, A.elements, MATRIX_SIZE * MATRIX_SIZE * sizeof(float), cudaMemcpyHostToDevice);
    ...
    while ( k < MATRIX_SIZE )
	{
        gauss_eliminate_division_kernel<<<1, TILE_SIZE>>>(U_d, k);
        ...
        gauss_eliminate_kernel<<<GRID_SIZE, TILE_SIZE>>>(U_d, k);
        cudaThreadSynchronize();
    ...
    gettimeofday(&stop, NULL);
    printf("GPU run time = %0.5f s. \n", (float)(stop.tv_sec - start.tv_sec + (stop.tv_usec - start.tv_usec)/(float)1000000));

    cudaMemcpy(U.elements, U_d, MATRIX_SIZE * MATRIX_SIZE * sizeof(float), cudaMemcpyDeviceToHost);
    cudaFree(U_d);
}
